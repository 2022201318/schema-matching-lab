# phase2_query_matches_no_aug.py
import os
import json
import asyncio
...
OUTPUT_FILE = os.path.join(DATA_ROOT, "final_matches_no_aug.json")
...
async def main():
    ...
    for domain_id, source_ids in domains.items():
        print(f"\n{'='*60}")
        print(f"🌐 查询 {domain_id}，数据源：{source_ids}")
        print(f"{'='*60}")
        ...
        if not os.path.exists(working_dir):
            print(f"⚠️  {domain_id} 索引目录不存在，跳过")
            continue
